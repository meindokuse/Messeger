package com.example.friendnet.ui.messages
...
import com.example.friendnet.databinding.FragmentChatBinding
import com.example.friendnet.models.MessageInChat
import com.example.friendnet.ui.MainActivity
import com.example.friendnet.ui.messages.rcview.MessagesLoadingState
import com.example.friendnet.ui.messages.rcview.MessagesPagingAdapter
import com.example.friendnet.ui.messages.viewmodel.MessagesViewModel
...
import com.example.friendnet.util.ServiceCallback
import com.example.friendnet.util.ServiceManager
import com.example.friendnet.util.Timer
...
class ChatFragment : Fragment() {
    ...
    private var audioBoundService: AudioService? = null

    private var isServiceBound = false

    val serviceCallback = object: ServiceCallback {
        override fun showPanel(title: String) {
            ActivityHolder.getCurrentActivity()?.showAudioPanel(title, audioBoundService!!)
        }

        override fun hidePanel() {
            ActivityHolder.getCurrentActivity()?.hideAudioPanel()
        }

        override fun updateProgressUiHolder(currentPosition:Int) {
            currentlyPlayingViewHolder?.updateProgress(currentPosition)
        }

        override fun updateUiHolderComplete() {
            currentlyPlayingViewHolder?.updateCompleteProgress(audioBoundService!!.getDurationMedia())
    ...
    val serviceConnection = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            ...
            audioBoundService = binder.getService()
            audioBoundService?.registerCallback(serviceCallback)
//            currentlyPlayingViewHolder?.initMediaPlayer(audioBoundService?.getMediaPlayer()!!)
            isServiceBound = true
            ServiceManager.setBoundService(audioBoundService!!)
        }

        override fun onServiceDisconnected(name: ComponentName?) {
            audioBoundService?.unregisterCallback()
            audioBoundService = null
    ...
    override fun onPause() {
        ...
        if (isServiceBound) {
            ...
            isServiceBound = false
            ServiceManager.deleteBoundService()
        }
    }
    ...
    private fun initButtons() {
        ...
        binding.sendTextMessageButton.setOnClickListener {
            ...
            val message = MessageInChat(mesId, userId, chatId, textForMessage, currentTime, 1,"0:00")
            try {
                ...
                try {
                    if (audioMessage == null) {
                        ...
                        val message = MessageInChat(
                            ...
                            currentTime,
                            2,
                            binding.recordTimeText.text.toString()
                        )
    ...
    private fun initRecyclerView() {
        ...
        pagingAdapter = MessagesPagingAdapter(object : AudioInterface {

            override fun onClickStartListen(position: Int) {

//                при старте нового холдера отменить играющий , внутри холдера будет будет переменная ( isActive )
                if (currentlyPlayingViewHolder == null){
                    ...
                    viewHolder?.let {
                        val content = viewHolder.exportAudioMessage?.content
                        lifecycleScope.launch{
                            val fileUri = async(Dispatchers.IO){ messageViewModel.getFilePathForHolder(requireContext(),chatId,content!!)  }.await()
                            Intent(requireContext(), AudioService::class.java).also {
                                activity?.startService(it)
                                activity?.bindService(it, serviceConnection, Context.BIND_AUTO_CREATE)
                            }
                            Log.e("AudioRecorder", "")
                            audioBoundService?.startPlayer("Голосовое сообщение", fileUri.toString())
                            val duration = audioBoundService?.getDurationMedia()
                            currentlyPlayingViewHolder?.initSeekBarProgress(duration!!)
                        }
                    }
                } else audioBoundService?.resumePlayer()


            }

            override fun onClickStopListen(position: Int) {
                Log.e("MyLog", "onClickStopListen")
                audioBoundService?.pausePlayer()
            }

            override fun seekMediaPlayerTo(progress:Int) {
                audioBoundService?.seekTo(progress)
            }

            override fun checkIsPlaying(): Boolean {
                return audioBoundService?.checkIsPlaying() ?: false
            }
