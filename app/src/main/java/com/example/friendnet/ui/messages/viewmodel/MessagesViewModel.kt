package com.example.friendnet.ui.messages.viewmodel
...
import com.example.friendnet.data.reposetory.message.MessagesMediator
import com.example.friendnet.data.reposetory.message.RemoteMessagesReposetoryImpl
import com.example.friendnet.domain.mapers.toMessageInChat
...
class MessagesViewModel @Inject constructor(
    ...
    suspend fun getFilePathForHolder(context: Context, chatId: String, fileName: String): Uri? {
        val file = File(context.filesDir, fileName)
        return if (file.exists()) file.toUri() else {
            val fileUri = remoteMessagesReposetory.getRemoteFile(chatId, fileName)
            fileUri?.let {
                fileManager.saveImageToInternalStorage(it,fileName)
            }
            fileUri
        }
    }
